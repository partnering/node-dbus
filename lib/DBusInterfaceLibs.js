...
function DBusMethod (ifaceClass = mandatory(), funcName = mandatory(), funcSignature = mandatory()) {
	...
	// Emit a warning message if the method doesn't begin with a capital letter
	if (funcName[0] !== funcName[0].toUpperCase()) {
		console.warn(`[WARNING]: DBus conventions is for method names to begin with capital letters ('${funcName}')`)
	}

	// Add the introspection data to the introspection structure (possibly updating / overriding previous value)
...
function DBusProperty (ifaceClass = mandatory(), propName = mandatory(), propSignature = mandatory()) {
	...
	// Emit a warning message if the property doesn't begin with a capital letter
	if (propName0[0] !== propName[0].toUpperCase()) {
		console.warn(`[WARNING]: DBus conventions is for property names to begin with capital letters ('${propName}')`)
	}
...
function DBusSignal (ifaceClass = mandatory(), sigName = mandatory(), sigSignature = mandatory()) {
	...
	// Emit a warning message if the signal doesn't begin with a capital letter
	if (sigName[0] !== sigName[0].toUpperCase()) {
		console.warn(`[WARNING]: DBus conventions is for signal names to begin with capital letters ('${sigName}')`)
	}
