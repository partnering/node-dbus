...
function DBusProxy (serviceName = mandatory(), bus = mandatory(), maxIntrospectionDepth = Infinity) {
	...
	/**
	 * The maximum levle of recursion to make in the introspection pass
	 * @type {number}
	 */
			...
			this.bus.signals.on(mangledSignalName, (args, _) => {
				...
				if (newOwner === '') {
					...
					*/
					...
					//TODO: should we make a new introspection pass here? i.e. is the service supposed to change interfaces? What about memory leaks?
					debug('Target came back online')
					this.emit('connected')
				}
			})
		})
								...
								// Otherwise, parse the arguments and check for standard signals
									...
									*/
									if (signalName === 'InterfacesAdded') {
										...
										this.makeIntrospectionPass(this.maxIntrospectionDepth, currObj, objPath)
										.then( _ => {
									...
									*/
									if (signalName === 'InterfacesRemoved') {
										...
										/*
											Check if we have received an empty array of removed interfaces, which means
											that the entire DBusObject was removed
										*/
										if (removedIfaces.length === 0) {
											// Then, remove the DBusObject
											parent.removeObject(targetPath)
										} else {
											// Then we must remove all supplied interfaces
