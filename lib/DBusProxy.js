 ...
 * @throws {TypeError}
 */
function DBusProxy (serviceName = mandatory(), targetedPath, bus = mandatory(), maxIntrospectionDepth = Infinity, isInterfaceUpdatingAllowed) {
	...
	this.isInterfaceUpdatingAllowed = isInterfaceUpdatingAllowed
	...
	.then( _ => {
		...
		return this.makeIntrospectionPass(maxIntrospectionDepth, targetedPath, this['/'], '/')
	})
	.then( _ => {
		/*
				...
				if (sName === serviceName) {
					if (newOwner === '') {
						...
						*/
						...
						delete this['/'].fr
						this.makeIntrospectionPass(maxIntrospectionDepth, targetedPath, this['/'], '/')
						.then( _ => {
...
*/
DBusProxy.prototype.makeIntrospectionPass = function (introspectionDepth, targetedPath, parent, path) {
		...
		.then( introspectData => {
			...
			for (let iface of ifaces) {
				...
				if (utils.isValidIfaceName (name)) {
					...
					*/
					if (isPropertiesIfacePresent && !excludedInterfaces.includes (name)) {
						...
						.then( _ => {
							...
							this.bus.signals.on(mangledSignalName, (msg, signature_) => {
								...
								// Otherwise, parse the arguments and check for standard signals
									...
									*/
									if (signalName === 'InterfacesAdded' && this.isInterfaceUpdatingAllowed) {
										...
										debug(`'InterfacesAdded' or 'InterfacesRemoved' detected. Making introspection pass at '${objPath}'`)
										this.makeIntrospectionPass(this.maxIntrospectionDepth, targetedPath, currObj, objPath)
										.then( _ => {
									...
									*/
									if (signalName === 'InterfacesRemoved' && this.isInterfaceUpdatingAllowed) {
										...
										const targetObj = targetPath.split('/').reduce( (acc, curr) => acc[curr], parent )
										...
										if (targetObj.getIfaceNames().length === 0) {
											debug(`All interfaces were removed, remove object.`)
			...
			for (let node of nodes) {
				...
				if (utils.isValidPathComponent (nodeName)) {
					...
					if (targetedPath != null) {
						if (!(targetedPath.indexOf(newPath) !== -1 || newPath.indexOf(targetedPath) !== -1)) {
							debug(`${newPath} doesn't match ${targetedPath}`)
							continue
						}
					} else {
						debug("No targeted object path")
					}
					recursiveNodes.push (this.makeIntrospectionPass (newDepth, targetedPath, parent[nodeName], newPath))
				} else {
