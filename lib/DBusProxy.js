		...
		.then( introspectData => {
								...
								// Otherwise, parse the arguments and check for standard signals
									...
									*/
									if (signalName === 'InterfacesAdded') {
										...
										if (depth > 0 && (this.areRelatedObjectPaths(targetedPath, objPath))) {
									...
									*/
									if (signalName === 'InterfacesRemoved') {
										...
										const targetObj = targetPath.split('/').reduce( (dBusObject, node) => {
											if (dBusObject != null && dBusObject[node] != null) {
												return dBusObject[node]
											}
											else {
												return null
											}
			...
			for (let node of nodes) {
				...
				if (utils.isValidPathComponent (nodeName)) {
					...
					? path + nodeName
					: path + '/' + nodeName
					if (!this.areRelatedObjectPaths(targetedPath, newPath)) {
 ...
 */
DBusProxy.prototype.areRelatedObjectPaths = function (path1, path2) {
