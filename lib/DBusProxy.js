		...
		.then( introspectData => {
			...
			for (let iface of ifaces) {
				...
				if (targetedIface != null) {
					if ((Array.isArray(targetedIface) && targetedIface.indexOf(iface.attr('name').value()) === -1) || (typeof targetedIface === 'string' && targetedIface !== iface.attr('name').value())) {
						debug(`In '${path}', '${iface.attr('name').value()}' doesn't match '[${targetedIface}]'`)
						continue
					} else {
						debug(`In '${path}', '${iface.attr('name').value()}' matches '[${targetedIface}]'`)
					}
				} else {
					debug(`In '${path}', no targeted interface`)
				}
								...
								// Otherwise, parse the arguments and check for standard signals
									...
									*/
									if (signalName === 'InterfacesAdded') {
										...
										if (depth > 0 && (this.pathsAreIncluded(targetedPath, objPath))) {
											...
											debug(`'InterfacesAdded' or 'InterfacesRemoved' detected. Making introspection pass at '${objPath}, depth: ${depth}`)
			...
			for (let node of nodes) {
				...
				if (utils.isValidPathComponent (nodeName)) {
					...
					? path + nodeName
					: path + '/' + nodeName
					if (!this.pathsAreIncluded(targetedPath, newPath)) {
...
DBusProxy.prototype.pathsAreIncluded = function (path1, path2) {
	if (path1 == null || path2 == null) return true
	let p1 = path1 !== '/' ? path1 + '/' : path1
	let p2 = path2 !== '/' ? path2 + '/' : path2
