...
function bus(conn, opts) {
    ...
    */
    ...
    /*
        ...
        It translated the new-API types to the old-API type so that it can be safely marshalled
    */
    this.invokeNewAPI = function(msg, callback) {
        const trees = signature(msg.signature)
        let translatedBody = []

        for (let idx in trees) {
            translatedBody.push(utils.fromNewToOldAPI(msg.body[idx], trees[idx]))
        }

        msg.proxy = true
        msg.body = translatedBody
       ...
       if (msg.type == constants.messageType.methodReturn || msg.type == constants.messageType.error) {
           ...
           if (handler) {
              ...
              if (msg.type == constants.messageType.methodReturn) {
                  ...
                  if (Array.isArray(msg.body) && msg.body.length === 1 && typeof msg.body[0] === 'string')
                    handler.call(props, new Error(msg.body))
                  else
            ...
            */
            if (self.newAPISignals.has(mangledName + 'NEWAPI')) {
                ...
                // Translate types from old to new API if there is a body if there is some argument in the body
                if (msg.signature !== undefined) {
                    trees = parseSignature (msg.signature)
                    translatedTypes = msg.body.map( (e, idx) => utils.fromOldToNewAPI (e, trees[idx]) )
                } else {
                    translatedTypes = undefined
                }
        ...
        */
        ...
        if (msg.type == constants.messageType.methodCall) {
            ...
            */
            ...
            .catch( (err) => {
                debug('Method call returned an error: ' + err)
     ...
     */
    this.mkProxy = function (serviceName = mandatory(), path, maxIntrospectionDepth = Infinity, isInterfaceUpdatingAllowed = true) {
        // We can't use 'promisify' here, because it complains that 'this.invokeDBus' is not a function
        // console.log('\nbus.mkProxy() called\n\n')
        return new DBusProxy(serviceName, path, self, maxIntrospectionDepth, isInterfaceUpdatingAllowed)
/*
...
//*/
    ...
    /*
        Make the bus object listen for 'NamedOwnerChanged' event, and provide a high-level, Javascript event for it.
        This is because it's a common use-case for people to have this, and it avoids them to make a Proxy.
    */
	this.on('ready', () => {
		const namedOwnerChangedRule = `type='signal',path='/org/freedesktop/DBus',interface='org.freedesktop.DBus',member='NameOwnerChanged'`

		this.addMatch(namedOwnerChangedRule, (err, _) => {
			// Emit an error if some reason we could not add the match rule
			if (err != null) {
				console.error(`Could not list to signal 'NamedOwnerChanged': ${err}`)
			} else {
				const mangledOwnerChanged = this.mangle('/org/freedesktop/DBus', 'org.freedesktop.DBus', 'NameOwnerChanged')

				this.signals.on(mangledOwnerChanged, (args, _) => {
					this.emit('NameOwnerChanged', ...args)
				})
			}
		})
	})
