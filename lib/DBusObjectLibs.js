...
class DBusObject {
	...
	removeObject (objPathComponent = mandatory()) {
		let obj
		if (this[objPathComponent] instanceof DBusObject) obj = this[objPathComponent]
		else throw new TypeError(`no object at ${objPathComponent}`)

		let inputObjQueue = [obj]
		let outputObjQueue = []
		while (objQueue.length) {
			let obj = objQueue.pop()
			inputObjQueue.push.apply(inputObjQueue, obj.getChildren())
			obj.removeAllInterfaces()
		}

		outputObjQueue.forEach(obj => obj._unlinkParentObject())
