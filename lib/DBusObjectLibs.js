...
class DBusObject {
	...
	removeInterfaces (ifaceNames = mandatory()) {
		...
		for (let ifaceName of ifaceNames) {
			try {
				this.removeInterface(ifaceName, false)
	...
	addObject (object = mandatory(), relativePath = mandatory()) {
		...
		if (objectManagerIface !== undefined) {
			...
			this.emitInterfacesAdded(objPath, ifacesAndProperties)
		} else {
	...
	removeObject (objPathComponent = mandatory()) {
		...
		// no '/' means the subtree is in our children
			...
			let allIfaces = this[objPathComponent].getIfaceNames()

			return this[objPathComponent]._removeObjectHierarchy()
			.then( _ => {
				let removedObjPath = ['', this.getPath(), objPathComponent].join('/')

				// Temporary sanitize (remove when getPath() bug is fixed)
				if (removedObjPath.startsWith('//'))
					removedObjPath = removedObjPath.substr(1)

				debug(`DBusObject '${objPathComponent}' removed, emitting now with objPath: '${removedObjPath}' and allIfaces: '${inspect(allIfaces)}'...`)
				...
				*/
				this.emitInterfacesRemoved(removedObjPath, allIfaces)
			})
			.then( _ => {
	 ...
	 * @todo: refactor code with 'InterfacesAdded'
	 */
	...
	/**
	 * Check if this DBusObject implements interface 'ObjectManager'. If yes, emit the 'InterfacesAdded' signal with
	 * the supplied arguments.<br>
	 * If no, forward this call to the parent object and recurse until a DBusObject that implements this interface is
	 * found, or the top-level is reached.
	 * @todo: refactor code with 'emitInterfacesRemoved'
	 */
	emitInterfacesAdded (objPath = mandatory(), ifacesAndProps = mandatory()) {
		const targetIfaceName = 'org.freedesktop.DBus.ObjectManager'
		const targetSignalName = 'InterfacesAdded'

		// Check if this DBusObject implements the interface
		if (!this.getIfaceNames().includes(targetIfaceName)) {
			// Forward the call to the parent object, if we have one
			if (this._parentObject !== undefined) {
				return this._parentObject.emitInterfacesAdded(objPath, ifacesAndProps)
			} else {
				debug(`Reached top-level and could not emit '${targetIfaceName}'`)
			}
		}
		// If we do, then emit the signal
		else {
			this[targetIfaceName].emit(targetSignalName, objPath, ifacesAndProps)
		}
	}
